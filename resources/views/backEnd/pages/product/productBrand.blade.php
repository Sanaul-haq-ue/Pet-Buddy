...
@section('adminContent')
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered align-items-center justify-content-center">
            <div class="modal-content rounded-4 shadow-lg border-0 w-50">
                ...
                <div class="modal-header border-0">
                    <div>
                        <h5 class="mb-0 fw-bold">Add New Product Brand</h5>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <!-- Form -->
                <form id="addProductBrandForm" class="modal-form">
                    @csrf
                    <div class="modal-body">
                        <!-- Product Brand Name -->
                        <div class="mb-3">
                            <label class="form-label">Product Brand Name</label>
                            <input type="text" name="productBrand_name" class="form-control"
                                placeholder="Enter product brand name" />
                            <div class="invalid-feedback text-danger text-xs mt-1" id="add_productBrand_name_error">
                            </div>
                        </div>
                        <!-- Status -->
                        <div class="mb-1">
    ...
    <div class="modal fade" id="editProductBrandModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered align-items-center justify-content-center">
            <div class="modal-content rounded-4 shadow-lg border-0 w-50">
                ...
                <div class="modal-header border-0">
                    <div>
                        <h5 class="mb-0 fw-bold">Edit Product Brand</h5>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <!-- Form -->
                <form id="editProductBrandForm" class="modal-form">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="productBrand_name" class="form-control"
                                placeholder="Enter product brand name" />
                            <div class="invalid-feedback text-danger text-xs mt-1" id="edit_productBrand_name_error">
                            </div>
                        </div>
                        <!-- Status -->
                        <div class="mb-1">
    ...
    <div class="modal fade" id="deleteProductBrandModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 rounded-4 shadow-lg">

                <div class="modal-header border-0">
                    <h5 class="modal-title text-danger md:text-2xl text-lg">Delete Product Brand</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                ...
                <div class="modal-footer border-0">
                    ...
                    <button type="button" class="btn btn-danger" id="confirmDeleteProductBrandBtn">
                        Yes, Delete
                    </button>
                </div>
    ...
    <div class="company-canvas">
        ...
        <div class="company-header d-flex justify-content-between align-items-end pb-4 mb-3">
            <div>
                <h3 class="fs-1 fw-bold text-on-surface tracking-tighter mb-1">Product Brand</h3>
            </div>
            <div class="d-flex gap-3">
                <button
                    ...
                    data-bs-toggle="modal" data-bs-target="#addProductBrandModal">
            ...
            <div class="col-12 col-md-12 d-flex flex-column gap-8">
                ...
                <div class="glass-card rounded-lg overflow-hidden d-flex flex-column">
                    <div class="d-flex justify-content-between align-items-center p-4 p-md-5 pb-3">
                        <h4 class="fs-5 font-headline fw-bold tracking-tight mb-0">Product Brand List</h4>
                                    ...
                                    <tr class="hover-bg-surface-lowest group">
                                        <td class="py-3 px-4 px-lg-5 align-middle">
                                            <div class="d-flex align-items-center gap-3">
                                                <span
                                                    class="fw-bold text-on-surface">{{ $ProductBrand->productBrand_name }}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-3 px-4 px-lg-5 align-middle text-end">
                                            ...
                                            <!-- Edit -->
                                            <button
                                                class="btn btn-link p-1 text-stone-400 hover-text-secondary text-decoration-none shadow-none edit-brand-btn"
                                                data-id="{{ $ProductBrand->id }}"
                                                data-name="{{ $ProductBrand->productBrand_name }}"
                                                data-status="{{ $ProductBrand->status }}" data-bs-toggle="modal"
                                            ...
                                            <!-- Delete -->
                                            <button type="button"
                                                class="btn btn-link p-1 text-stone-400 hover-text-error text-decoration-none shadow-none delete-brand-btn"
                                                data-id="{{ $ProductBrand->id }}">
    ...
    <!-- AJAX scripts for Product Brand CRUD -->
    <script>
        $(document).ready(function() {
            ...
            // AJAX submit for Add Brand Form
            $('#addProductBrandForm').on('submit', function(e) {
                e.preventDefault();
            ...
            // Populate Edit Brand Modal
            $(document).on('click', '.edit-brand-btn', function() {
                var id = $(this).data('id');
                var name = $(this).data('name');
                var status = $(this).data('status');

                var form = $('#editProductBrandForm');
                ...
                // Populate fields
                $('#editBrandId').val(id);
                form.find('input[name="productBrand_name"]').val(name);
                form.find('input[name="status"][value="' + status + '"]').prop('checked', true);
            });

            // AJAX submit for Edit Brand Form
            $('#editProductBrandForm').on('submit', function(e) {
                e.preventDefault();
            ...
            // Confirm Delete
            $('#confirmDeleteProductBrandBtn').on('click', function() {
                if (!deleteCategoryId) {
                    return;
                }
                ...
                $.ajax({
                    url: "{{ route('productBrand.delete', ':id') }}".replace(':id', deleteCategoryId),
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        const modalElement = document.getElementById('deleteProductBrandModal');
